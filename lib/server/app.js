...
app.put('/', function (req, res) {
  if (!req.body || !req.body._id || !req.body.title || !req.body.content) {
    ...
    db.documentIndex.save(di, console.log);
    ...
    Object.keys(c).map(function (s) {
      ...
      db.stemIndex.find({ _id: s }, function (err, stem) {
        ...
        if (stem[0]) {
          ...
          if (found) { // This will only happen on update
            console.log('document found in stem index, updating');
            db.stemIndex.update({ '_id': s, 'e._id': di._id }, { '$set': { 'e.$.count': c[s] } }, console.log);
          } else {
            console.log('document not found in stem index, pushing');
            db.stemIndex.update({ '_id': s }, { '$push': { 'e': si } }, console.log);
          }
        } else {
          console.log('stem index not found, creating');
          db.stemIndex.save({ '_id': s, 'e': [ si ] }, console.log);
        }
      });
    });
